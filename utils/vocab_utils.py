...
def index_words(lang_model, lmdb_dir):
    ...
    for key, buf in cursor:
        ...
        #print(video.keys())#['vid', 'clips', 'framerate']
        for clip in video['clips']:
            #print(clip.keys())#dict_keys(['words', 'skeletons', 'skeletons_3d', 'audio_feat', 'audio_raw', 'start_frame_no', 'end_frame_no', 'start_time', 'end_time'])
            #print('Duration is: ', int(clip['end_frame_no']) - int(clip['start_frame_no']))
            #print(len(clip['words']))
            print(clip['words'])
            for word_info in clip['words']:
                word = word_info[0]
                #print('word is: ', word_info)
