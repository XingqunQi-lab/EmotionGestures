...
class alignment(object):
    ...
    def load_audio(self, wave, t_start, without_file=False, sr_audio=16000):

        short_y = wave[t_start*sr_audio:]
        self.oenv = librosa.onset.onset_strength(y=short_y, sr=sr_audio)
        #print('1')
    ...
    def load_pose(self, pose, t_start, t_end, pose_fps, without_file=False):
        ...
        # l2
        for line_data_np in pose: #,args.pre_frames, args.pose_length
            data_each_file.append(np.concatenate([line_data_np[18:42], line_data_np[150:174], ],0))
        data_each_file = np.array(data_each_file)
        #print('data_each_file shape is: ', data_each_file.shape)
        vel= data_each_file[1:, :] - data_each_file[:-1, :]        
        
        
        
        
        vel_right_shoulder = np.linalg.norm(np.array([vel[:, 0], vel[:, 1], vel[:, 2], vel[:, 3], vel[:,4], vel[:, 5]]), axis=0)
        vel_right_arm = np.linalg.norm(np.array([vel[:, 6], vel[:, 7], vel[:, 8], vel[:, 9], vel[:,10], vel[:, 11]]), axis=0)
        vel_right_fore_arm = np.linalg.norm(np.array([vel[:, 12], vel[:, 13], vel[:, 14], vel[:, 15], vel[:,16], vel[:, 17]]), axis=0)
        vel_right_wrist = np.linalg.norm(np.array([vel[:, 18], vel[:, 19], vel[:, 20], vel[:, 21], vel[:,22], vel[:, 23]]), axis=0)
        ...
        beat_right_fore_arm = argrelextrema(vel_right_fore_arm[t_start*pose_fps:t_end*pose_fps], np.less, order=self.order)
        beat_right_wrist = argrelextrema(vel_right_wrist[t_start*pose_fps:t_end*pose_fps], np.less, order=self.order)
        vel_left_shoulder = np.linalg.norm(np.array([vel[:, 24], vel[:, 25], vel[:, 26], vel[:, 27], vel[:,28], vel[:, 29]]), axis=0)
        vel_left_arm = np.linalg.norm(np.array([vel[:, 30], vel[:, 31], vel[:, 32], vel[:, 33], vel[:,34], vel[:, 35]]), axis=0)
        vel_left_fore_arm = np.linalg.norm(np.array([vel[:, 36], vel[:, 37], vel[:, 38], vel[:, 39], vel[:,40], vel[:, 41]]), axis=0)
        vel_left_wrist = np.linalg.norm(np.array([vel[:, 42], vel[:, 43], vel[:, 44], vel[:, 45], vel[:,46], vel[:, 47]]), axis=0)
        ...
        beat_left_fore_arm = argrelextrema(vel_left_fore_arm, np.less, order=self.order)
        beat_left_wrist = argrelextrema(vel_left_wrist, np.less, order=self.order)
        return beat_right_arm, beat_right_shoulder, beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist
    ...
    def load_data(self, wave, pose, t_start, t_end, pose_fps):
        ...
        return onset_raw, onset_bt, onset_bt_rms, beat_right_arm, beat_right_shoulder,  beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist 

    def eval_random_pose(self, wave, pose, t_start, t_end, pose_fps, num_random=60):
        onset_raw, onset_bt, onset_bt_rms = self.load_audio(wave, t_start, t_end)
        dur = t_end - t_start
        for i in range(num_random):
            beat_right_arm, beat_right_shoulder, beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist = self.load_pose(pose, i, i+dur, pose_fps)
            dis_all_b2a= self.calculate_align(onset_raw, onset_bt, onset_bt_rms, beat_right_arm, beat_right_shoulder, beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist)
    ...
    def calculate_align(self, onset_raw, onset_bt, onset_bt_rms, beat_right_arm, beat_right_shoulder, beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist, pose_fps=15):
        # more stable solution
        avg_dis_all_b2a = 0
        
        for audio_beat in [onset_raw, onset_bt, onset_bt_rms]:
            for pose_beat in [beat_right_arm, beat_right_shoulder, beat_right_fore_arm, beat_right_wrist, beat_left_arm, beat_left_shoulder, beat_left_fore_arm, beat_left_wrist]:
