...
class alignment(object):
    ...
    def load_pose(self, pose, t_start, t_end, pose_fps, without_file=False):
        ...
        # l2
        for line_data_np in pose: #,args.pre_frames, args.pose_length
            data_each_file.append(np.concatenate([line_data_np[18:36], line_data_np[150:168], ],0))
        data_each_file = np.array(data_each_file)
        ...
        vel_right_wrist = np.linalg.norm(np.array([vel[:, 12], vel[:, 13], vel[:, 14], vel[:, 15], vel[:,16], vel[:, 17]]), axis=0)
        ...
        vel_left_shoulder = np.linalg.norm(np.array([vel[:, 18], vel[:, 19], vel[:, 20], vel[:, 21], vel[:,22], vel[:, 23]]), axis=0)
        vel_left_arm = np.linalg.norm(np.array([vel[:, 24], vel[:, 25], vel[:, 26], vel[:, 27], vel[:,28], vel[:, 29]]), axis=0)
        vel_left_wrist = np.linalg.norm(np.array([vel[:, 30], vel[:, 31], vel[:, 32], vel[:, 33], vel[:,34], vel[:, 35]]), axis=0)
