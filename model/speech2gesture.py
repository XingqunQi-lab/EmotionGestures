...
class AudioEncoder(nn.Module):
    ...
    def forward(self, spectrogram):
        ...
        out = self.first_net(spectrogram) #torch.Size([32, 256, 14, 7])
        #print('out after first audio shape is: ', out.shape)
        out = self.make_1d(out) #torch.Size([32, 256, 34, 1])
        #print('out after second audio shape is: ', out.shape)
        x1 = out.squeeze(3)
...
class Generator(nn.Module):
    ...
    def forward(self, in_spec, pre_poses):
        #in_spec (32, 128, 70)
        audio_feat_seq = self.audio_encoder(in_spec)  # output (bs, feat_size, n_frames) torch.Size([32, 256, 34])
        #print('audio_feat_seq shape is: ', audio_feat_seq.shape )
        pre_poses = pre_poses.reshape(pre_poses.shape[0], -1)#torch.Size([32, 456])
        #print('pre_poses shape is: ', pre_poses.shape ) 
        pre_pose_feat = self.pre_pose_encoder(pre_poses)  # output (bs, 16)# torch.Size([32, 16])
        #print('pre_pose_feat shape is: ', pre_pose_feat.shape ) 
        pre_pose_feat = pre_pose_feat.unsqueeze(2).repeat(1, 1, self.gen_length) #torch.Size([32, 16, 34])
        #print('pre_pose_feat shape is: ', pre_pose_feat.shape )
        feat = torch.cat((audio_feat_seq, pre_pose_feat), dim=1) #torch.Size([32, 272, 34])
        #print('feat shape is: ', feat.shape )
        out = self.decoder(feat) #torch.Size([32, 256, 34])
...
if __name__ == '__main__':
    # for model debugging
    pose_dim = 38 * 3
    generator = Generator(34, pose_dim, 4)
    spec = torch.randn((32, 128, 70)) # B, dim, T(sequence)
    pre_poses = torch.randn((32, 4, pose_dim))
